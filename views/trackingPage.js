...
function getStatusMeta(displayStatus, shipment) {
  ...
  if (stage === "registered") {
    return {
      ...
      description:
        ...
        "Försändelsen är registrerad hos transportören men ännu inte inlämnad.",
  ...
  if (stage === "received") {
    return {
      ...
      description:
        displayStatus.description ||
        "Transportören har tagit emot försändelsen.",
  ...
  if (stage === "in_transit") {
    return {
      ...
      description:
        displayStatus.description ||
        "Försändelsen är på väg genom transportnätverket.",
  ...
  if (stage === "ready_for_pickup") {
    return {
      ...
      description:
        displayStatus.description ||
        "Försändelsen kan nu hämtas ut av mottagaren.",
  ...
  if (stage === "delivered") {
    return {
      ...
      description:
        displayStatus.description ||
        "Försändelsen har levererats till mottagaren.",
  ...
  if (stage === "issue") {
    return {
      ...
      description:
        displayStatus.description ||
        "Det finns en avvikelse i leveransflödet.",
  ...
  if (shipmentStatus === "completed") {
    return {
      label: "Skickad",
      description:
        "Försändelsen är skapad och skickad vidare med transportören.",
  ...
  if (shipmentStatus === "failed") {
    return {
      label: "Misslyckades",
      description:
        "Något gick fel i leveransflödet och behöver kontrolleras.",
      badgeClass: "status-failed"
    };
  }
...
function renderCarrierStatusMessage(carrierTracking, displayStatus) {
  ...
  if (carrierTracking.success && carrierTracking.statusText) {
    const mappedLabel = displayStatus?.label
      ? ` • Tolkad status: ${escapeHtml(displayStatus.label)}`
      : "";

    return `
      <div class="carrier-box carrier-success">
        Live PostNord-status: ${escapeHtml(carrierTracking.statusText)}${mappedLabel}
      </div>
    `;
  }
  ...
  if (carrierTracking.skipped && carrierTracking.reason) {
    return `
      <div class="carrier-box carrier-warning">
        Live PostNord-tracking används inte just nu: ${escapeHtml(
  ...
  if (!carrierTracking.success && carrierTracking.reason) {
    return `
      <div class="carrier-box carrier-warning">
        Kunde inte hämta live PostNord-tracking just nu: ${escapeHtml(
          carrierTracking.reason
        )}
      </div>
    `;
...
function getTimelineState(displayStatus, shipment) {
  const stage = displayStatus?.stage || "";
  const shipmentDone = Boolean(shipment?.created_at);
  const fulfillmentDone = Boolean(shipment?.completed_at);

  const transportRegistered =
    stage === "registered" ||
    stage === "received" ||
    stage === "in_transit" ||
...
function renderTrackingPage(data) {
  ...
  const displayStatus = data?.displayStatus || null;
  ...
  const headingText = escapeHtml(getHeadingText(displayStatus, shipment));
  const statusMeta = getStatusMeta(displayStatus, shipment);
  const timeline = getTimelineState(displayStatus, shipment);
        ...
        <div class="wrap">
          ...
          <div class="card">
            <div class="hero">
              <div class="hero-grid">
                ...
                <div class="status-panel">
                  ...
                  <div class="status-badge ${statusMeta.badgeClass}">${escapeHtml(
                    statusMeta.label
                  )}</div>
                  <p class="status-description">${escapeHtml(
            ...
            <div class="content">
              ...
              ${renderCarrierStatusMessage(carrierTracking, displayStatus)}
              ...
              <div class="main-grid">
                ...
                <div class="section-card">
                  ...
                  <ul class="timeline">
                    <li class="timeline-item ${timeline.shipmentDone ? "done" : ""}">
                    ...
                    <li class="timeline-item ${timeline.fulfillmentDone ? "done" : ""}">
                      <div class="timeline-dot"></div>
                      <div>
                        <div class="timeline-title">Fulfillment</div>
                        <div class="timeline-value">${
                          shipment.completed_at ? completedAt : "Inte slutförd ännu"
                        }</div>
                      </div>
                    </li>

                    <li class="timeline-item ${
                      timeline.transportRegistered ? "active" : ""
                    }">
                      <div class="timeline-dot"></div>
                      <div>
                        <div class="timeline-title">Transport registrerad</div>
                        <div class="timeline-value">${
                          timeline.transportRegistered
                            ? "Transportören har registrerat försändelsen."
                            : "Väntar på carrier-bekräftelse."
                        }</div>
                      </div>
                    </li>

                    <li class="timeline-item ${
                      timeline.transportMoving ? "active" : ""
                    }">
                      <div class="timeline-dot"></div>
                      <div>
                        <div class="timeline-title">Under transport</div>
                        <div class="timeline-value">${
                          timeline.transportMoving
                            ? "Försändelsen är i rörelse genom transportnätverket."
                            : "Försändelsen har ännu inte kommit så långt."
                        }</div>
                      </div>
                    </li>

                    <li class="timeline-item ${timeline.delivered ? "done" : ""}">
                      <div class="timeline-dot"></div>
                      <div>
                        <div class="timeline-title">Levererad</div>
                        <div class="timeline-value">${
                          timeline.delivered
                            ? "Försändelsen har levererats."
                            : "Leverans är ännu inte bekräftad."
                        }</div>
                      </div>
                    </li>
                  </ul>
              ...
              <div class="footer">
                ShipOne visar nu interna tracking-händelser tillsammans med live PostNord-händelser och en förbättrad statusmappning för tracking-sidan.
              </div>
            </div>
          </div>
        </div>
